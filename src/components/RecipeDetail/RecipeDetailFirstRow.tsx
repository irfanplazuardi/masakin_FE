...
const RecipeDetailFirstRow: React.FC<RecipeDetailFirstRowProps> = ({ onIngredientsClick, onToolsClick, onStepsClick }) => {
  const [activeButton, setActiveButton] = useState<'ingredients' | 'tools' | 'steps' | null>(null);
  ...
  return (
    <div className="flex items-center justify-between w-full">
      ...
      <button
        className={`py-2 px-4 rounded-lg flex justify-center items-center ${activeButton === 'tools' ? 'bg-green-600 text-white' : 'text-green-600'}`}
        onClick={handleToolsClick}
        style={{ width: '106px', height: '33px', borderRadius: '10px', backgroundColor: activeButton === 'tools' ? '#7E9F10' : 'transparent', color: activeButton !== 'tools' ? '#7E9F10' : '#FFFFFF' }}
      >
        <span className="font-poppins text-xs font-semibold whitespace-nowrap">
          Alat-alat
        </span>
