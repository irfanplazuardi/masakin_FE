...
const Recipe = () => {
  ...
  return (
    <div>
      ...
      <RecipeReview rating={rating} cookingTime={cookingTime} difficultyLevel={difficultyLevel} maxDifficultyLevel={maxDifficultyLevel} />
      ...
      {/* Konten Bahan-bahan */}
      {showIngredients && <div className="bg-white py-4 px-6">{/* Konten bahan-bahan */}</div>}

      {/* Konten Alat-alat */}
      {showTools && <div className="bg-white py-4 px-6">{/* Konten alat-alat */}</div>}

      {/* Konten Cara Masak */}
      {showSteps && <div className="bg-white py-4 px-6">{/* Konten langkah-langkah memasak */}</div>}
      <Navbar />
    </div>
  );
};
